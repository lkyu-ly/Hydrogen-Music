...
<template>
    <div class="list-item" @click.stop="showFiles(item)" :class="{'list-item-open': item.show && item.children.length != 0, 'list-item-selected': router.currentRoute.value.query.name == item.name}" v-for="(item, index) in props.folderChildren" :key="index">
        <div class="folder" >
